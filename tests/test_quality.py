...
def test_leading_missing_values_inside_profile_window_are_unusable():
    ...
    assert result["profile_missing_percent"] == 20.0


def test_missing_before_profile_window_does_not_make_meter_unusable():
    values = pd.Series([None] * 20 + [10.0] * 80)
    ...
    assert result["missing_percent"] == 20.0

    assert result["profile_missing_percent"] == 0.0
